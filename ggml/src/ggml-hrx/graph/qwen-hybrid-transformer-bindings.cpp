...
static bool checked_multiply(int64_t lhs, int64_t rhs, int64_t & result) {
    ...
    if (product < std::numeric_limits<int64_t>::min() || product > std::numeric_limits<int64_t>::max()) {
        return false;
    }
...
static ValueId append_scratch(Graph & graph, const std::string & name, size_t byte_length) {
    ...
    value.name           = "hrx.synthetic.qwen_hybrid." + name;
...
class RecipeEvaluator {
  public:
    RecipeEvaluator(const Graph & graph, const QwenHybridRecipeMatch & match, std::vector<std::string> & errors) :
        graph_(graph),
        match_(match),
        definition_(*match.definition),
        errors_(errors) {
        seed_tensors();
        seed_attributes();
    }
    ...
    bool scalar(const nlohmann::json & expression, ScalarValue & result) { return evaluate(expression, result); }
  ...
  private:
    ...
    void set_integer(const std::string & name, int64_t value) {
        ...
        if (!inserted && (position->second.kind != ScalarValue::Kind::Integer || position->second.integer != value)) {
    ...
    void set_floating(const std::string & name, double value) {
        ...
        if (!inserted && (position->second.kind != ScalarValue::Kind::Floating || position->second.floating != value)) {
            errors_.push_back(context() + ": incoherent fact " + name);
        }
    }
    ...
    void seed_tensors() {
        ...
        for (auto schema = schemas.begin(); schema != schemas.end(); ++schema) {
            ...
            if (mapped == match_.values_by_role.end() || mapped->second >= graph_.values.size()) {
                if (schema.value().value("optional", false)) {
                    continue;
                }
            ...
            for (size_t dimension = 0; dimension < value.access.shape.size(); ++dimension) {
                ...
                if (value.access.shape[dimension] < 0 ||
                    ...
                    errors_.push_back(context() + ": tensor " + schema.key() + " has an invalid element count");
                } else {
                ...
                if (element_size == 0 || value.access.strides[dimension] % element_size != 0) {
                    errors_.push_back(context() + ": tensor " + schema.key() + " has a non-integral element stride");
                } else {
            ...
            const auto shape_entry = schema.value().find("shape");
            if (shape_entry == schema.value().end()) {
                continue;
            }
            const nlohmann::json & shape = *shape_entry;
            ...
            for (size_t dimension = 0; dimension < shape.size(); ++dimension) {
                if (shape[dimension].is_string()) {
                    set_integer(shape_prefix + shape[dimension].get<std::string>(), value.access.shape[dimension]);
    ...
    void seed_attributes() {
        for (auto operation = definition_.at("attributes").begin(); operation != definition_.at("attributes").end();
             ++operation) {
    ...
    bool evaluate(const nlohmann::json & expression, ScalarValue & result) {
        ...
        if (expression.is_string()) {
            return evaluate_name(expression.get<std::string>(), result);
        }
        ...
        if (expression.contains("field")) {
            return evaluate(expression.at("field"), result);
        }
        if (expression.contains("value")) {
            return evaluate(expression.at("value"), result);
        }
        if (expression.contains("product") || expression.contains("sum")) {
            ...
            for (const nlohmann::json & operand : operands) {
                int64_t value = 0;
                if (!evaluate_integer_operand(operand, value)) {
                    return false;
                }
                int64_t    next = 0;
                const bool valid =
                    product ? checked_multiply(accumulator, value, next) : checked_add(accumulator, value, next);
                if (!valid) {
        ...
        if (expression.contains("ceil_div")) {
            ...
            if (!evaluate_integer_operand(operands[0], numerator) ||
                !evaluate_integer_operand(operands[1], denominator)) {
                return false;
            }
    ...
    const Graph &                      graph_;
    const QwenHybridRecipeMatch &      match_;
...
static std::string format_scalar(const ScalarValue &        value,
    ...
    if (type == "f32") {
        const double floating =
            value.kind == ScalarValue::Kind::Floating ? value.floating : static_cast<double>(value.integer);
        if (std::isfinite(floating) &&
            (floating < -std::numeric_limits<float>::max() || floating > std::numeric_limits<float>::max())) {
            errors.push_back(context + ": value does not fit f32");
            return {};
        }
        const float narrowed = static_cast<float>(floating);
        if (std::isnan(narrowed)) {
            out << "nan";
        } else if (std::isinf(narrowed)) {
            out << (narrowed < 0 ? "-inf" : "inf");
        } else {
            out << std::setprecision(std::numeric_limits<float>::max_digits10) << narrowed;
        }
    } else if (type == "i32") {
        if (value.kind != ScalarValue::Kind::Integer || value.integer < std::numeric_limits<int32_t>::min() ||
...
static bool encode_launch_scalar(const ScalarValue &        value,
                                 const std::string &        type,
                                 int64_t &                  encoded,
                                 const std::string &        context,
                                 std::vector<std::string> & errors) {
    ...
    if (type == "i32" &&
        (value.integer < std::numeric_limits<int32_t>::min() || value.integer > std::numeric_limits<int32_t>::max())) {
...
VerificationResult materialize_qwen_hybrid_recipe(Graph &                       graph,
    ...
    if (!recipe.definition) {
        result.errors.push_back("native Qwen hybrid recipe has no physical definition");
        return result;
    }
    ...
    for (auto item = recipe.definition->at("transient_buffers").begin();
        ...
        if (!evaluator.integer(item.value().at("size"), byte_length)) {
            continue;
        }
        if (byte_length <= 0 || static_cast<uint64_t>(byte_length) > std::numeric_limits<size_t>::max()) {
            result.errors.push_back("native recipe " + recipe.recipe + ": invalid transient size for " + item.key());
            continue;
        }
    ...
    for (const nlohmann::json & physical : recipe.definition->at("dispatches")) {
        ...
        for (const nlohmann::json & buffer : buffers) {
            ...
            if (buffer.contains("tensor")) {
                ...
                if (mapped == recipe.values_by_role.end()) {
                    result.errors.push_back("native recipe " + recipe.recipe + ": unknown tensor binding " + tensor);
                    continue;
                }
                binding.value = mapped->second;
                const auto schema = recipe.definition->at("tensor_schema").find(tensor);
                if (schema != recipe.definition->at("tensor_schema").end()) {
                    binding.layout = schema.value().value("storage", std::string());
                }
                ...
                if (mapped == transients.end()) {
                    result.errors.push_back("native recipe " + recipe.recipe + ": unknown transient binding " +
                                            transient);
                    continue;
                }
                binding.value = mapped->second;
            ...
            auto materialize_range = [&](const char * field, size_t & value) {
                if (!buffer.contains(field)) {
                    return true;
                }
                int64_t evaluated = 0;
                if (!evaluator.integer(buffer.at(field), evaluated)) {
                    return false;
                }
                if (evaluated < 0 || static_cast<uint64_t>(evaluated) > std::numeric_limits<size_t>::max()) {
                    result.errors.push_back("native recipe " + recipe.recipe + ": invalid buffer " + field);
                    return false;
                }
                value = static_cast<size_t>(evaluated);
                return true;
            };
            if (!materialize_range("offset", binding.offset) || !materialize_range("length", binding.length)) {
                continue;
            }
        ...
        for (const nlohmann::json & binding : config.at("bindings")) {
            ...
            if (binding.contains("source")) {
                ...
                ScalarValue       value;
                if (!evaluator.scalar(source, value)) {
                    continue;
                }
                dispatch.kernel.compile_parameters[name] =
                    format_scalar(value, type, "native recipe " + recipe.recipe + " config " + name, result.errors);
            } else if (binding.contains("value")) {
                ScalarValue value;
                if (!evaluator.scalar(binding.at("value"), value)) {
                    continue;
                }
                dispatch.kernel.compile_parameters[name] =
                    format_scalar(value, type, "native recipe " + recipe.recipe + " config " + name, result.errors);
            } else {
                result.errors.push_back("native recipe " + recipe.recipe + ": config " + name +
                                        " has no source or value");
            }
        }
        ...
        for (const nlohmann::json & scalar : scalars) {
            ...
            ScalarValue       value;
            if (scalar.contains("source")) {
                if (!evaluator.scalar(scalar.at("source"), value)) {
                    continue;
                }
            } else if (scalar.contains("value")) {
                if (!evaluator.scalar(scalar.at("value"), value)) {
                    continue;
                }
            } else {
                result.errors.push_back("native recipe " + recipe.recipe + ": scalar " + name +
                                        " has no source or value");
                continue;
            }
            int64_t encoded = 0;
            if (encode_launch_scalar(value, type, encoded, "native recipe " + recipe.recipe + " scalar " + name,
                                     result.errors)) {
    ...
    if (!invocation.dispatches.empty()) {
        invocation.kernel = invocation.dispatches.front().kernel;
    }
    return result;
}
