...
static Schedule atom_schedule(const Graph & graph) {
    ...
    for (const Operation & operation : graph.operations) {
        ...
        invocation.kernel.kernel_id =
            kernel_catalog_id(invocation.kernel.family.c_str(), invocation.kernel.variant.c_str());
        ...
        if (dispatch_ordinal != 0) {
            dispatch.dependencies.push_back(dispatch_ordinal - 1);
        }
...
bool transitional_program_adapter::bind_storage_roots(const graph_plan &                       source,
                                                      const ProgramPlan &                      program,
                                                      const std::vector<const ggml_tensor *> & source_roots,
                                                      std::vector<const ggml_tensor *> &       program_roots,
                                                      std::vector<std::string> &               errors) {
    if (source_roots.size() != source.storages().size()) {
        errors.push_back("source storage-root bindings do not cover the Graph2 plan");
        return false;
    }
    if (program.graph.storages.size() < source.storages().size() ||
        program.resources.resources.size() != program.graph.storages.size()) {
        errors.push_back("transitional program storage table does not extend the Graph2 plan");
        return false;
    }
    for (size_t i = 0; i < source.storages().size(); ++i) {
        const graph_storage & source_storage  = source.storages()[i];
        const graph_storage & program_storage = program.graph.storages[i];
        if (source_storage.id != program_storage.id || source_storage.root != program_storage.root ||
            source_storage.size != program_storage.size || source_storage.external != program_storage.external ||
            source_storage.weight != program_storage.weight ||
            source_storage.mutable_state != program_storage.mutable_state ||
            source_storage.final_version != program_storage.final_version) {
            errors.push_back("transitional program changed source storage " + std::to_string(i));
            return false;
        }
    }
    for (size_t i = source.storages().size(); i < program.resources.resources.size(); ++i) {
        if (!program.resources.resources[i].elidable) {
            errors.push_back("transitional program added non-elidable storage " + std::to_string(i));
            return false;
        }
    }
    program_roots.assign(program.graph.storages.size(), nullptr);
    std::copy(source_roots.begin(), source_roots.end(), program_roots.begin());
    return true;
}

bool eager_capability_declared(enum ggml_op op) {
    return primitive_capability_declared(op);
}
...
ResourceProgram build_resource_program(const Graph & graph, const Schedule & schedule) {
    ...
    for (ValueId root : graph.roots) {
        if (root < graph.values.size()) {
            root_storages.insert(graph.values[root].access.storage);
        }
    }
    ...
    for (const Value & value : graph.values) {
        if (value.access.storage < result.resources.size()) {
            result.resources[value.access.storage].aliases.push_back(value.id);
        }
    }
    for (uint32_t invocation_id = 0; invocation_id < schedule.invocations.size(); ++invocation_id) {
        ...
        auto merge_use = [&](StorageId storage, uint32_t before_version, uint32_t after_version,
        ...
        for (OperationId operation_id : invocation.covered_operations) {
            ...
            for (const Effect & effect : operation.effects) {
                const ResourceAccess access =
                    effect.kind == EffectKind::Write ? ResourceAccess::Write : ResourceAccess::Read;
        ...
        for (const Dispatch & dispatch : invocation.dispatches) {
            for (const TensorBinding & binding : dispatch.bindings) {
                if (binding.value >= graph.values.size()) {
                    continue;
                }
...
VerificationResult verify_resource_program(const Graph &           graph,
    ...
    for (size_t i = 0; i < resources.resources.size(); ++i) {
        ...
        if (resource.storage != i || resource.size != storage.size) {
            append_error(result, "resource identity or size mismatch");
        }
        if (storage.external && !resource.imported) {
            append_error(result, "external storage is not imported");
        }
        if (storage.mutable_state && !resource.exported) {
            append_error(result, "mutable storage is not exported");
        }
        if (resource.elidable && (resource.imported || resource.exported)) {
            append_error(result, "boundary storage is marked elidable");
        }
        for (ValueId alias : resource.aliases) {
            if (alias >= graph.values.size() || graph.values[alias].access.storage != i) {
                append_error(result, "resource alias escapes its storage root");
            }
        }
    }
    ...
    for (const ResourceUse & use : resources.uses) {
        ...
        if (use.invocation < last_invocation) {
            append_error(result, "resource uses are not invocation ordered");
        }
        ...
        if (use.before_version > use.after_version || use.before_version > graph.storages[use.storage].final_version ||
            use.after_version > graph.storages[use.storage].final_version) {
            append_error(result, "resource use has an invalid storage version");
        }
        observed_use[use.storage]     = true;
        observed_version[use.storage] = std::max(observed_version[use.storage], use.after_version);
    }
    for (const Storage & storage : graph.storages) {
        if (!observed_use[storage.id]) {
            append_error(result, "logical storage has no scheduled use");
        }
        if (observed_version[storage.id] != storage.final_version) {
            append_error(result, "resource program does not reach the final storage version");
        }
    }
    return result;
}
...
std::string schedule_semantic_witness(const Graph & graph, const Schedule & schedule) {
    ...
    auto value_witness = [&](ValueId value_id) {
        ...
        for (int64_t dimension : value.access.shape) {
            value_out << dimension << ',';
        }
        value_out << "]p";
        if (value.producer == kInvalidId) {
            value_out << "external";
        } else {
            value_out << graph.operations[value.producer].original_ordinal;
        }
    ...
    for (const Operation & operation : graph.operations) {
        ...
        for (ValueId input : operation.inputs) {
            out << value_witness(input) << ';';
        }
        out << "->" << value_witness(operation.output) << ':';
        for (uint8_t byte : operation.raw_params) {
    ...
    for (const Invocation & invocation : schedule.invocations) {
        ...
        out << '\n';
        for (const TensorBinding & input : invocation.inputs) {
            out << "in:" << input.role << ':' << value_witness(input.value) << '\n';
        }
        for (const TensorBinding & output : invocation.outputs) {
            out << "out:" << output.role << ':' << value_witness(output.value) << '\n';
        }
        for (const Dispatch & dispatch : invocation.dispatches) {
            out << execution_kind_name(dispatch.kernel.execution_kind) << ':'
                << kernel_specialization_name(dispatch.kernel);
    ...
    for (const RootContract & root : schedule.roots) {
        out << "root:" << root_disposition_name(root.disposition) << ':' << root.replacement << '\n';
    }
    return out.str();
}
...
ProgramPlan build_reactive_plan(const Graph & graph, const std::string & target) {
    ...
    if (structural.structurally_recognized) {
        ...
        result.atom_fallback_count =
            std::count_if(result.schedule.invocations.begin(), result.schedule.invocations.end(),
    ...
    const VerificationResult resource_verification =
        verify_resource_program(result.graph, result.schedule, result.resources);
...
ProgramPlan build_transitional_program(const graph_plan &        plan,
    ...
    if (!plan.valid() || !selection.valid()) {
        ProgramPlan result;
        result.target = target;
        result.errors.insert(result.errors.end(), plan.diagnostics().errors.begin(), plan.diagnostics().errors.end());
        result.errors.insert(result.errors.end(), selection.errors.begin(), selection.errors.end());
        for (const missing_program_step & missing : selection.missing) {
            result.errors.push_back("program selection has no implementation for region " +
                                    std::to_string(missing.region) + ": " + missing.reason);
        }
        return result;
    }
    transitional_graph graph = transitional_graph::from_plan(plan);
    graph.fingerprint        = "uid-" + std::to_string(graph_uid);
    if (plan.facts().count("layer_count") != 0) {
        const GraphIndex             index(graph);
        const RoutedTransformerModel logical = RoutedTransformerModel::from_plan(plan, index);
        if (!logical.valid()) {
            ProgramPlan result;
            result.graph  = std::move(graph);
            result.target = target;
            result.errors = logical.errors;
            return result;
        }
    }
    ProgramPlan result = build_reactive_plan(graph, target);
    if (result.valid()) {
        result.planner_identity = "graph2/" + result.planner_identity;
    }
    return result;
}
