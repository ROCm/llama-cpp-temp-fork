#include "transitional-fusion-search.h"
...
class FusionSearchImplementation {
  public:
    static bool same_fact_value(const FactValue & lhs, const FactValue & rhs) { return lhs == rhs; }
    ...
    static bool better_score(const CandidateScore & lhs, const CandidateScore & rhs) {
        const auto left  = std::make_tuple(lhs.evidence == CostEvidenceKind::Measured, lhs.primary_benefit,
                                           lhs.secondary_benefit, lhs.covered_operations);
        const auto right = std::make_tuple(rhs.evidence == CostEvidenceKind::Measured, rhs.primary_benefit,
                                           rhs.secondary_benefit, rhs.covered_operations);
        return left > right;
    }
    ...
    struct WorseQueueEntry {
        bool operator()(const QueueEntry & lhs, const QueueEntry & rhs) const {
            if (!equal_score(lhs.score, rhs.score)) {
                return better_score(rhs.score, lhs.score);
            }
            return lhs.key > rhs.key;
        }
    };

    static void record(SearchResult &          result,
    ...
    static std::string event_kind_name(SearchEventKind kind) {
        switch (kind) {
            case SearchEventKind::Seeded:
                return "seeded";
            case SearchEventKind::Rejected:
                return "rejected";
            case SearchEventKind::Popped:
                return "popped";
            case SearchEventKind::Expanded:
                return "expanded";
            case SearchEventKind::Stale:
                return "stale";
            case SearchEventKind::Invalidated:
                return "invalidated";
            case SearchEventKind::Committed:
                return "committed";
        }
        return "unknown";
    }
...
Decision FactDatabase::observe(std::string key, FactValue value, FactEvidence evidence) {
    ...
    if (!FusionSearchImplementation::same_fact_value(fact.value, value)) {
        ...
        for (const FactEvidence & prior : fact.evidence) {
            if (prior.graph_id != kInvalidId) {
                ids.push_back(prior.graph_id);
            }
        }
        if (evidence.graph_id != kInvalidId) {
            ids.push_back(evidence.graph_id);
        }
        return Decision::reject(DecisionReason::InconsistentFact,
                                "fact " + fact.key +
                                    " disagrees: " + FusionSearchImplementation::fact_value_text(fact.value) +
                                    " versus " + FusionSearchImplementation::fact_value_text(value),
                                std::move(ids));
    }
    fact.evidence.push_back(std::move(evidence));
    return Decision::allow();
...
Decision FusionProvider::discover(const GraphIndex &, FactDatabase &) const {
    return Decision::allow();
}

void FusionProvider::expand(const GraphIndex &,
                            const FactDatabase &,
...
void PlannerConfiguration::add_provider(std::shared_ptr<const FusionProvider> provider) {
    ...
    const auto        duplicate   = std::find_if(providers_.begin(), providers_.end(),
                                                 [&](const auto & existing) { return existing->id() == provider_id; });
    if (duplicate != providers_.end()) {
        return;
    }
    providers_.push_back(std::move(provider));
    std::sort(providers_.begin(), providers_.end(),
              [](const auto & lhs, const auto & rhs) { return std::string(lhs->id()) < std::string(rhs->id()); });
}

std::string PlannerConfiguration::identity() const {
    std::ostringstream out;
    for (const auto & provider : providers_) {
        out << provider->id() << '@' << provider->revision() << ';';
    }
    return out.str();
}
...
CandidateScore FusionCandidate::score(const FusionCandidate & candidate) {
    ...
    if (result.evidence == CostEvidenceKind::Measured) {
        ...
        result.secondary_benefit =
            candidate.economics.eliminated_materialization_bytes - candidate.economics.additional_scratch_bytes;
    }
    return result;
}

SearchResult SearchResult::search(const GraphIndex &           index,
    ...
    std::priority_queue<FusionSearchImplementation::QueueEntry, std::vector<FusionSearchImplementation::QueueEntry>,
                        FusionSearchImplementation::WorseQueueEntry>
                          queue;
    ...
    auto insert_candidate = [&](FusionCandidate candidate, size_t provider_index, SearchEventKind event_kind) {
        ...
        if (candidate.key.empty()) {
            candidate.key = candidate.provider + ":" + candidate.family + ":" +
                            (candidate.hero == kInvalidId ? std::string("none") : index.structural_key(candidate.hero));
        }
        if (!candidate_keys.insert(candidate.key).second) {
            return;
        }
        const Decision legality =
            index.validate_region(candidate.operations, candidate.materialized_outputs, candidate.allow_disconnected);
        ...
        if (!legality.allowed || (!score.positive() && !candidate.correctness_baseline)) {
            ...
            Decision rejection = legality.allowed ? Decision::reject(DecisionReason::NoComparableCost,
                                                                     "candidate has no positive comparable benefit") :
                                                    legality;
            FusionSearchImplementation::record(result, options, SearchEventKind::Rejected, candidate, score,
                                               std::move(rejection));
            return;
        }
        ...
        if (event_kind == SearchEventKind::Seeded) {
            ++result.report.seeded;
        } else {
    ...
    for (size_t provider_index = 0; provider_index < configuration.providers().size(); ++provider_index) {
        ...
        for (FusionCandidate & seed : seeds) {
            insert_candidate(std::move(seed), provider_index, SearchEventKind::Seeded);
        }
    ...
    while (!queue.empty()) {
        ...
        if (entry.candidate >= candidates.size() || !active[entry.candidate] ||
            ...
            if (entry.candidate < candidates.size()) {
                FusionSearchImplementation::record(result, options, SearchEventKind::Stale, candidates[entry.candidate],
                                                   entry.score,
        ...
        FusionSearchImplementation::record(result, options, SearchEventKind::Popped, candidate, entry.score,
                                           Decision::allow());
        ...
        if (overlap != candidate.operations.end()) {
            ...
            FusionSearchImplementation::record(
                result, options, SearchEventKind::Invalidated, candidate, entry.score,
        ...
        if (!expanded[entry.candidate]) {
            ...
            configuration.providers()[provider_for_candidate[entry.candidate]]->expand(index, result.facts, candidate,
                                                                                       expansions);
            ...
            for (FusionCandidate & expansion : expansions) {
                insert_candidate(std::move(expansion), provider_for_candidate[entry.candidate],
                                 SearchEventKind::Expanded);
            }
        ...
        for (OperationId operation : candidate.operations) {
            claimed[operation] = 1;
        }
        result.selected.push_back(candidate);
        active[entry.candidate] = 0;
        ++result.report.committed;
        FusionSearchImplementation::record(result, options, SearchEventKind::Committed, candidate, entry.score,
                                           Decision::allow());
        ...
        for (size_t other = 0; other < candidates.size(); ++other) {
            ...
            if (overlaps) {
                ...
                ++result.report.invalidated;
                FusionSearchImplementation::record(
                    result, options, SearchEventKind::Invalidated, candidates[other],
                    FusionCandidate::score(candidates[other]),
    ...
    for (OperationId operation = 0; operation < claimed.size(); ++operation) {
        if (!claimed[operation]) {
            result.uncovered_operations.push_back(operation);
        }
    }
    ...
    for (const FusionCandidate & candidate : result.selected) {
        regions.push_back(candidate.operations);
    }
    ...
    for (size_t position : order) {
        ordered.push_back(std::move(result.selected[position]));
    }
...
std::string SearchResult::format_report(const SearchResult & result) {
    std::ostringstream out;
    out << "fusion-search valid=" << (result.valid() ? "yes" : "no") << " facts=" << result.facts.facts().size()
        << " selected=" << result.selected.size() << " uncovered=" << result.uncovered_operations.size() << '\n';
    ...
    for (const auto & item : result.facts.facts()) {
        out << "fact " << item.first << '=' << FusionSearchImplementation::fact_value_text(item.second.value)
            << " evidence=";
        for (const FactEvidence & evidence : item.second.evidence) {
            out << evidence.source;
            if (evidence.graph_id != kInvalidId) {
                out << '#' << evidence.graph_id;
            }
            out << ',';
        }
        out << '\n';
    ...
    for (const FusionCandidate & candidate : result.selected) {
        ...
        out << "select " << candidate.key << " family=" << candidate.family
            << " components=" << candidate.logical_components.size() << " operations=" << candidate.operations.size()
            << " dispatches=" << candidate.economics.planned_dispatches << '/'
            << candidate.economics.reference_dispatches << " score=" << score.primary_benefit << '/'
            << score.secondary_benefit << '\n';
    }
    for (const SearchEvent & event : result.report.events) {
        ...
        if (!event.decision.detail.empty()) {
            out << " detail=" << event.decision.detail;
        }
        out << '\n';
    }
...
std::string SearchResult::serialize_report_json(const SearchResult & result) {
    ...
    for (const auto & item : result.facts.facts()) {
        nlohmann::ordered_json fact = {
            { "key",      item.first                                                     },
            { "value",    FusionSearchImplementation::fact_value_text(item.second.value) },
    ...
    root["counters"] = {
        { "seeded",      result.report.seeded      },
        { "rejected",    result.report.rejected    },
        { "popped",      result.report.popped      },
        { "stale",       result.report.stale       },
        { "expanded",    result.report.expanded    },
        { "invalidated", result.report.invalidated },
        { "committed",   result.report.committed   },
    };
    return root.dump();
}
