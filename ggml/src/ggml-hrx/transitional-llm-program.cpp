...
class RoutedTransformerAnalysisImplementation {
  public:
    ...
    static OperationId unique_consumer(const GraphIndex &         index,
                                       ValueId                    value,
                                       enum ggml_op               kind,
        ...
        for (OperationId candidate : index.consumers(value)) {
            if (index.graph().operations[candidate].op != kind) {
                continue;
            }
    ...
    static OperationId trace_layout_consumer(const GraphIndex &         index,
                                             ValueId                    value,
                                             enum ggml_op               target,
                                             std::vector<std::string> & errors,
                                             const std::string &        role) {
    ...
    static OperationId find_ancestor(const Graph & graph, ValueId value, enum ggml_op kind) {
        ...
        while (!worklist.empty()) {
            ...
            if (operation == kInvalidId) {
                continue;
            }
            if (graph.operations[operation].op == kind) {
                return operation;
            }
    ...
    static std::set<OperationId> ancestors_within(const GraphIndex &               index,
        ...
        for (OperationId root : roots) {
            if (allowed.count(root) != 0 && result.insert(root).second) {
                worklist.push(root);
            }
        }
        while (!worklist.empty()) {
            ...
            for (OperationId predecessor : index.predecessors(current)) {
                if (allowed.count(predecessor) != 0 && result.insert(predecessor).second) {
    ...
    static bool has_ancestor_in(const GraphIndex & index, OperationId root, const std::set<OperationId> & candidates) {
        ...
        while (!worklist.empty()) {
            ...
            worklist.pop();
            for (OperationId predecessor : index.predecessors(current)) {
                if (candidates.count(predecessor) != 0) {
                    return true;
                }
    ...
    static void add_component(RoutedTransformerBlock &       block,
    ...
    static bool observe(FactDatabase &      facts,
...
RoutedTransformerModel RoutedTransformerModel::analyze(const GraphIndex & index) {
    ...
    for (const Operation & operation : graph.operations) {
        if (operation.op == GGML_OP_FLASH_ATTN_EXT) {
            flash_operations.push_back(operation.id);
        }
    }
    ...
    for (OperationId flash : flash_operations) {
        ...
        const OperationId flash_reshape = RoutedTransformerAnalysisImplementation::unique_consumer(
            index, flash_op.output, GGML_OP_RESHAPE, model.errors, "attention result reshape");
        if (flash_reshape == kInvalidId) {
            return model;
        }
        const OperationId attention_projection = RoutedTransformerAnalysisImplementation::unique_consumer(
            index, graph.operations[flash_reshape].output, GGML_OP_MUL_MAT, model.errors,
            "attention output projection");
        if (attention_projection == kInvalidId) {
            return model;
        }
        const OperationId attention_residual = RoutedTransformerAnalysisImplementation::trace_layout_consumer(
            index, graph.operations[attention_projection].output, GGML_OP_ADD, model.errors, "attention residual");
        if (attention_residual == kInvalidId) {
            return model;
        }
        ...
        const bool first_is_attention = RoutedTransformerAnalysisImplementation::find_ancestor(
                                            graph, residual_op.inputs[0], GGML_OP_MUL_MAT) == attention_projection;
        ...
        const bool        selects_attention =
            attention_adapter != kInvalidId && graph.operations[attention_adapter].op == GGML_OP_GET_ROWS &&
            !graph.operations[attention_adapter].inputs.empty() &&
            RoutedTransformerAnalysisImplementation::producer(graph, graph.operations[attention_adapter].inputs[0]) !=
                kInvalidId;
        const bool selects_hidden =
            hidden_adapter != kInvalidId && graph.operations[hidden_adapter].op == GGML_OP_GET_ROWS &&
            !graph.operations[hidden_adapter].inputs.empty() &&
            RoutedTransformerAnalysisImplementation::producer(graph, graph.operations[hidden_adapter].inputs[0]) !=
                kInvalidId;
        ...
        const OperationId hidden_input_producer =
            RoutedTransformerAnalysisImplementation::producer(graph, hidden_input);
        ...
        if (feed_forward_norm == kInvalidId) {
            return model;
        }
        const OperationId feed_forward_prepared = RoutedTransformerAnalysisImplementation::unique_consumer(
            index, graph.operations[feed_forward_norm].output, GGML_OP_MUL, model.errors,
            "feed-forward prepared input");
        if (feed_forward_prepared == kInvalidId) {
            return model;
        }
        const OperationId router_projection = RoutedTransformerAnalysisImplementation::unique_consumer(
            index, graph.operations[feed_forward_prepared].output, GGML_OP_MUL_MAT, model.errors, "router projection");
        if (router_projection == kInvalidId) {
            return model;
        }
        const OperationId router_softmax = RoutedTransformerAnalysisImplementation::unique_consumer(
            index, graph.operations[router_projection].output, GGML_OP_SOFT_MAX, model.errors, "router softmax");
        if (router_softmax == kInvalidId) {
            return model;
        }
        const OperationId router_argsort = RoutedTransformerAnalysisImplementation::unique_consumer(
            index, graph.operations[router_softmax].output, GGML_OP_ARGSORT, model.errors, "router argsort");
        if (router_argsort == kInvalidId) {
            return model;
        }
        const OperationId route_ids = RoutedTransformerAnalysisImplementation::unique_consumer(
            index, graph.operations[router_argsort].output, GGML_OP_VIEW, model.errors, "route identifiers");
        if (route_ids == kInvalidId) {
            return model;
        }

        OperationId gate_up = kInvalidId;
        for (const Operation & operation : graph.operations) {
            if (operation.op != GGML_OP_GLU || operation.inputs.size() < 2) {
                continue;
            }
            ...
            if (gate == kInvalidId || up == kInvalidId || graph.operations[gate].op != GGML_OP_MUL_MAT_ID ||
                graph.operations[up].op != GGML_OP_MUL_MAT_ID) {
                continue;
            }
        ...
        const OperationId routed_down = RoutedTransformerAnalysisImplementation::unique_consumer(
            index, graph.operations[gate_up].output, GGML_OP_MUL_MAT_ID, model.errors, "routed down projection");
        if (routed_down == kInvalidId) {
            return model;
        }
        ...
        const std::set<OperationId> block_set =
            RoutedTransformerAnalysisImplementation::block_closure(index, final_residual, hidden_input_producer);
        ...
        const OperationId query_projection =
            RoutedTransformerAnalysisImplementation::find_ancestor(graph, flash_op.inputs[0], GGML_OP_MUL_MAT);
        ...
        const OperationId attention_prepared =
            RoutedTransformerAnalysisImplementation::producer(graph, attention_prepared_value);
        ...
        const OperationId attention_norm =
            RoutedTransformerAnalysisImplementation::producer(graph, graph.operations[attention_prepared].inputs[0]);
        ...
        for (OperationId consumer : index.consumers(attention_prepared_value)) {
            if (graph.operations[consumer].op == GGML_OP_MUL_MAT) {
                qkv_projections.push_back(consumer);
            }
        }
        ...
        OperationId query_rope =
            RoutedTransformerAnalysisImplementation::find_ancestor(graph, flash_op.inputs[0], GGML_OP_ROPE);
        ...
        for (OperationId operation : block_set) {
            if (graph.operations[operation].op == GGML_OP_SET_ROWS) {
                cache_writers.push_back(operation);
            }
        }
        ...
        for (OperationId writer : cache_writers) {
            ...
            if (writer_op.inputs.empty()) {
                continue;
            }
            const OperationId projection =
                RoutedTransformerAnalysisImplementation::find_ancestor(graph, writer_op.inputs[0], GGML_OP_MUL_MAT);
            const OperationId rope = RoutedTransformerAnalysisImplementation::find_primary_ancestor_before_matmul(
                graph, writer_op.inputs[0], GGML_OP_ROPE);
            if (projection == kInvalidId) {
                continue;
            }
            if (rope != kInvalidId) {
        ...
        if (key_projection == kInvalidId || value_projection == kInvalidId) {
            model.errors.push_back("cannot distinguish key and value projection paths in block " +
                                   std::to_string(block.ordinal) + " writers=" + std::to_string(cache_writers[0]) +
                                   ',' + std::to_string(cache_writers[1]) + " key=" + std::to_string(key_projection) +
                                   " value=" + std::to_string(value_projection));
            return model;
        }

        const OperationId gate_projection =
            RoutedTransformerAnalysisImplementation::producer(graph, graph.operations[gate_up].inputs[0]);
        const OperationId up_projection =
            RoutedTransformerAnalysisImplementation::producer(graph, graph.operations[gate_up].inputs[1]);
        ...
        for (OperationId operation : block_set) {
            if (graph.operations[operation].op != GGML_OP_RESHAPE) {
                continue;
            }
            ...
            if (!inputs.empty()) {
                ...
                if (input_producer != kInvalidId && graph.operations[input_producer].op == GGML_OP_DIV) {
                    route_weights = operation;
                }
            }
        }
        ...
        block.operations_by_role.attention_output_selection   = selects_attention ? attention_adapter : kInvalidId;
        block.operations_by_role.hidden_state_selection       = selects_hidden ? hidden_adapter : kInvalidId;
        ...
        const std::set<OperationId> prepare =
            RoutedTransformerAnalysisImplementation::ancestors_within(index, { attention_prepared }, block_set);
        std::vector<OperationId> publication_roots = cache_writers;
        publication_roots.push_back(query_rope);
        const std::set<OperationId> qkv_cumulative =
            RoutedTransformerAnalysisImplementation::ancestors_within(index, publication_roots, block_set);
        const std::set<OperationId> qkv = RoutedTransformerAnalysisImplementation::difference(qkv_cumulative, prepare);
        const std::set<OperationId> flash_cumulative =
            RoutedTransformerAnalysisImplementation::ancestors_within(index, { flash, flash_reshape }, block_set);
        std::set<OperationId> claimed = prepare;
        claimed.insert(qkv.begin(), qkv.end());
        const std::set<OperationId> attention =
            RoutedTransformerAnalysisImplementation::difference(flash_cumulative, claimed);
        const std::set<OperationId> output_cumulative =
            RoutedTransformerAnalysisImplementation::ancestors_within(index, { feed_forward_prepared }, block_set);
        claimed.insert(attention.begin(), attention.end());
        // Recompute after including the attention component.
        const std::set<OperationId> output_component =
            RoutedTransformerAnalysisImplementation::difference(output_cumulative, claimed);
        claimed.insert(output_component.begin(), output_component.end());
        const std::set<OperationId> router_cumulative =
            RoutedTransformerAnalysisImplementation::ancestors_within(index, { route_ids, route_weights }, block_set);
        const std::set<OperationId> router =
            RoutedTransformerAnalysisImplementation::difference(router_cumulative, claimed);
        claimed.insert(router.begin(), router.end());
        const std::set<OperationId> gate_cumulative =
            RoutedTransformerAnalysisImplementation::ancestors_within(index, { gate_up }, block_set);
        const std::set<OperationId> gate =
            RoutedTransformerAnalysisImplementation::difference(gate_cumulative, claimed);
        ...
        RoutedTransformerAnalysisImplementation::add_component(block, RoutedTransformerComponentKind::AttentionPrepare,
                                                               attention_prepared, prepare);
        RoutedTransformerAnalysisImplementation::add_component(
            block, RoutedTransformerComponentKind::AttentionQkvPublication, query_projection, qkv);
        RoutedTransformerAnalysisImplementation::add_component(block, RoutedTransformerComponentKind::Attention, flash,
                                                               attention);
        RoutedTransformerAnalysisImplementation::add_component(
            block, RoutedTransformerComponentKind::AttentionOutputPrepare, attention_projection, output_component);
        RoutedTransformerAnalysisImplementation::add_component(block, RoutedTransformerComponentKind::RouterSelection,
                                                               router_projection, router);
        RoutedTransformerAnalysisImplementation::add_component(block, RoutedTransformerComponentKind::ExpertGateUp,
                                                               gate_up, gate);
        RoutedTransformerAnalysisImplementation::add_component(
            block, RoutedTransformerComponentKind::ExpertDownPublication, routed_down, down);
    ...
    for (OperationId projection : index.consumers(first.values_by_role.attention_prepared)) {
        if (graph.operations[projection].op != GGML_OP_MUL_MAT || projection == first_query.id) {
            continue;
        }
        const int64_t width = graph.values[graph.operations[projection].output].access.shape[0];
        if (model.key_value_size == 0 || width < model.key_value_size) {
            model.key_value_size = width;
        }
    }
    model.expert_count =
        graph.values[graph.operations[first.operations_by_role.router_projection].output].access.shape[0];
    model.route_count        = graph.values[first.values_by_role.router_route_ids].access.shape[0];
    model.output_token_count = 1;
    for (ValueId root : graph.roots) {
        if (root < graph.values.size()) {
            model.output_token_count = std::max<int64_t>(model.output_token_count, graph.values[root].access.shape[1]);
        }
    }
    ...
    for (OperationId operation = 0; operation < raised.size(); ++operation) {
        ...
        } else if (raised[operation] != 1) {
            model.errors.push_back("logical operation " + std::to_string(operation) +
...
const char * RoutedTransformerModel::component_kind_name(RoutedTransformerComponentKind kind) {
    switch (kind) {
        case RoutedTransformerComponentKind::ProgramPreamble:
        ...
        case RoutedTransformerComponentKind::AttentionQkvPublication:
            return "attention.qkv_publication";
        ...
        case RoutedTransformerComponentKind::AttentionOutputPrepare:
            return "attention.output_prepare";
        ...
        case RoutedTransformerComponentKind::ExpertGateUp:
            return "experts.gate_up";
        case RoutedTransformerComponentKind::ExpertDownPublication:
            return "experts.down_publication";
        case RoutedTransformerComponentKind::ProgramEndpoint:
            return "program.endpoint";
        case RoutedTransformerComponentKind::Atom:
            return "atom";
    }
    return "unknown";
}
...
std::string RoutedTransformerModel::format(const RoutedTransformerModel & model) {
    ...
    auto component = [&](const RoutedTransformerComponent & value, int64_t block) {
        out << "component " << value.id << " kind=" << component_kind_name(value.kind) << " block=" << block
            << " hero=" << value.hero << " ops=";
        for (OperationId operation : value.operations) {
            out << operation << ',';
        }
        out << " inputs=";
        for (ValueId input : value.boundary.inputs) {
            out << input << ',';
        }
        out << " outputs=";
        for (ValueId output : value.boundary.outputs) {
            out << output << ',';
        }
        out << '\n';
    };
    ...
    for (const RoutedTransformerBlock & block : model.blocks) {
        for (const RoutedTransformerComponent & value : block.components) {
            component(value, block.ordinal);
        }
    }
    ...
    if (!model.unraised_operations.empty()) {
        out << "unraised=";
        for (OperationId operation : model.unraised_operations) {
            out << operation << ',';
        }
        out << '\n';
    }
...
std::string RoutedTransformerModel::serialize_json(const RoutedTransformerModel & model) {
    ...
    out << "{\"version\":1,\"graph_fingerprint\":\"" << escape(model.graph_fingerprint)
        ...
        << ",\"key_value_token_count\":" << model.key_value_token_count << ",\"hidden_size\":" << model.hidden_size
        << ",\"query_size\":" << model.query_size << ",\"key_value_size\":" << model.key_value_size
        << ",\"expert_count\":" << model.expert_count << ",\"route_count\":" << model.route_count
        << "},\"components\":[";
    bool first     = true;
    auto component = [&](const RoutedTransformerComponent & value, int64_t block) {
        if (!first) {
        ...
        first = false;
        out << "{\"id\":" << value.id << ",\"kind\":\"" << component_kind_name(value.kind) << "\",\"block\":" << block
            << ",\"hero\":" << value.hero << ",\"operations\":[";
...
VerificationResult RoutedTransformerModel::verify(const GraphIndex & index, const RoutedTransformerModel & model) {
    ...
    auto                 verify_component = [&](const RoutedTransformerComponent & component) {
        ...
        if (!legality.allowed) {
            result.errors.push_back("logical component " + std::string(name) + " is illegal: " + legality.detail);
        }
    };
    ...
    for (const RoutedTransformerComponent & fallback : model.fallback_components) {
        ...
        for (OperationId operation : fallback.operations) {
            if (operation < fallback_owners.size()) {
                ++fallback_owners[operation];
            }
        }
    }
    for (OperationId operation = 0; operation < owners.size(); ++operation) {
        const bool declared_unraised = std::find(model.unraised_operations.begin(), model.unraised_operations.end(),
                                                 operation) != model.unraised_operations.end();
...
Decision RoutedTransformerProvider::discover(const GraphIndex & index, FactDatabase & facts) const {
    ...
    const RoutedTransformerModel & model =
        supplied_model_ ? *supplied_model_ : (recovered_model = RoutedTransformerModel::analyze(index));
    if (!model.valid()) {
        return Decision::reject(DecisionReason::ProviderError,
    ...
    if (!RoutedTransformerAnalysisImplementation::observe(facts, "llm.layer_count", model.blocks.size(),
                                                          "routed blocks", hero, failure) ||
        !RoutedTransformerAnalysisImplementation::observe(facts, "llm.query_token_count", model.query_token_count,
                                                          "attention input", hero, failure) ||
        !RoutedTransformerAnalysisImplementation::observe(facts, "llm.output_token_count", model.output_token_count,
                                                          "graph roots", hero, failure) ||
        !RoutedTransformerAnalysisImplementation::observe(facts, "llm.key_value_token_count",
                                                          model.key_value_token_count, "flash key", hero, failure) ||
        !RoutedTransformerAnalysisImplementation::observe(facts, "llm.hidden_size", model.hidden_size,
                                                          "attention input", hero, failure) ||
        !RoutedTransformerAnalysisImplementation::observe(facts, "llm.query_size", model.query_size, "query projection",
                                                          hero, failure) ||
        !RoutedTransformerAnalysisImplementation::observe(facts, "llm.key_value_size", model.key_value_size,
                                                          "key/value projections", hero, failure) ||
        !RoutedTransformerAnalysisImplementation::observe(facts, "llm.expert_count", model.expert_count,
                                                          "router projection", hero, failure) ||
        !RoutedTransformerAnalysisImplementation::observe(facts, "llm.route_count", model.route_count,
                                                          "router selection", hero, failure)) {
        return failure;
    }
    ...
    for (const RoutedTransformerBlock & block : model.blocks) {
        ...
        if (!RoutedTransformerAnalysisImplementation::observe(facts, "llm.hidden_size", block_prepared.access.shape[0],
                                                              "block attention input", block_hero, failure) ||
            !RoutedTransformerAnalysisImplementation::observe(facts, "llm.query_token_count",
                                                              block_prepared.access.shape[1], "block attention input",
                                                              block_hero, failure)) {
            return failure;
        }
    }
...
void RoutedTransformerProvider::seed(const GraphIndex & index,
                                     const FactDatabase &,
    ...
    auto       dispatches_for = [&](RoutedTransformerComponentKind kind) -> int {
        if (decode) {
            if (kind == RoutedTransformerComponentKind::AttentionPrepare) {
                return 0;
            }
            if (kind == RoutedTransformerComponentKind::AttentionQkvPublication) {
                return 2;
            }
            if (kind == RoutedTransformerComponentKind::Attention) {
                return 1;
            }
            if (kind == RoutedTransformerComponentKind::AttentionOutputPrepare) {
                return 3;
            }
            if (kind == RoutedTransformerComponentKind::RouterSelection) {
                return 2;
            }
            if (kind == RoutedTransformerComponentKind::ExpertGateUp) {
                return 2;
            }
            if (kind == RoutedTransformerComponentKind::ExpertDownPublication) {
                return 2;
            }
        } else {
            if (kind == RoutedTransformerComponentKind::AttentionPrepare) {
    ...
    auto append = [&](const std::string & family, const std::string & key, LogicalComponentId component_id,
                      OperationId hero, const std::vector<OperationId> & operations, int planned_dispatches,
    ...
    if (!model.preamble_operations.empty()) {
        append("program.preamble", "preamble", model.preamble.id, model.preamble_operations.back(),
               model.preamble_operations, decode ? 3 : 2, true);
    }
    for (const RoutedTransformerBlock & block : model.blocks) {
        for (size_t component_ordinal = 0; component_ordinal < block.components.size(); ++component_ordinal) {
            ...
            append(role, "block." + std::to_string(block.ordinal) + '.' + role, component.id, component.hero,
                   component.operations, 1);
    ...
    if (!model.endpoint_operations.empty()) {
        append("program.endpoint", "endpoint", model.endpoint.id, model.endpoint_operations.front(),
               model.endpoint_operations, decode ? 1 : 2, true);
    }
    for (const RoutedTransformerComponent & fallback : model.fallback_components) {
...
void RoutedTransformerProvider::expand(const GraphIndex & index,
                                       const FactDatabase &,
    ...
    auto alternative = [&](const char * recipe, int planned_dispatches, std::vector<OperationId> operations = {}) {
        if (!catalog_.contains(recipe)) {
            return;
        }
    ...
    if (payload->decode) {
        if (kind == RoutedTransformerComponentKind::AttentionQkvPublication) {
            alternative(routed_transformer_recipes::kDecodeQkvPostprocess, 1);
        }
        if (kind == RoutedTransformerComponentKind::Attention && payload->following_component != kInvalidId &&
            catalog_.contains(routed_transformer_recipes::kDecodeAttentionNextQ8)) {
            std::vector<OperationId> grown =
                union_operations(candidate.operations, payload->following_component_operations);
        ...
        if (kind == RoutedTransformerComponentKind::RouterSelection) {
            alternative(routed_transformer_recipes::kDecodeRouterTopK, 1);
        }
        if (kind == RoutedTransformerComponentKind::ExpertGateUp) {
            alternative(routed_transformer_recipes::kDecodeGateUpNextQ8, 1);
        }
        if (kind == RoutedTransformerComponentKind::ExpertDownPublication && !payload->terminal &&
            ...
            std::vector<OperationId> grown =
                union_operations(candidate.operations, payload->following_prepare_operations);
...
PlannerConfiguration RoutedTransformerProvider::make_planner(
    ...
    configuration.add_provider(
        std::make_shared<RoutedTransformerProvider>(std::move(catalog), std::move(supplied_model)));
