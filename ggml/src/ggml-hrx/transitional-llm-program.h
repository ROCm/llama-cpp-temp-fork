...
struct RoutedTransformerModel {
    ...
    static RoutedTransformerModel from_plan(const graph_plan & plan, const GraphIndex & index);
...
class RoutedTransformerProvider final : public FusionProvider {
  public:
    explicit RoutedTransformerProvider(RoutedTransformerRecipeCatalog                catalog        = {},
    ...
    void                        expand(const GraphIndex &             index,
                                       const FactDatabase &           facts,
                                       const FusionCandidate &        candidate,
                                       std::vector<FusionCandidate> & expansions) const override;
    static PlannerConfiguration make_planner(RoutedTransformerRecipeCatalog                catalog        = {},
                                             std::shared_ptr<const RoutedTransformerModel> supplied_model = {});
